...
def subscription_status(user):
    ...
    trial_expiry = (
        datetime.fromisoformat(user["subscription_expires_at"])
    ...
    # Active promotional trial must be both:
    # 1. a vendor/Fast Food account
    # 2. still within its expiration date
    # 3. still on the basic plan
    trial_active = (
        is_vendor_role
        and trial_expiry
        and trial_expiry > now
        and user["plan"] == "basic"
    )

    # Paid Premium Store access is separate from the promotional trial.
    premium_active = (
        is_vendor_role
    ...
    if trial_active:
        return {
            ...
            "is_premium": False,
            "trial": True,
            "expires": trial_expiry.strftime("%d %b %Y")
        }
...
def admin_configured():
    return bool(get_admin_username() and get_admin_password()) or bool(
        query_db("SELECT id FROM admin_users LIMIT 1")
    )
...
def get_vendor_categories(user_id):
    return [
        row["category"]
        for row in query_db(
            "SELECT category FROM vendor_categories WHERE user_id = ? ORDER BY category",
            (user_id,)
        )
    ]
...
def home():
    if request.method == "POST":
        ...
        # 👑 ACTIVE PREMIUM ACCESS = PAID PREMIUM OR ACTIVE TRIAL
        premium_access = (
            vendor_subscription.get("is_premium", False)
            or vendor_subscription.get("trial", False)
        )

        listing_count_row = query_db(
            "SELECT COUNT(*) AS count FROM products WHERE seller = ?",
            (session["username"],),
            one=True
        )
        ...
        # 👑 TRIAL ACCOUNTS RECEIVE THE SAME UNLIMITED LISTING ACCESS
        # AS PAID PREMIUM ACCOUNTS.
        if not premium_access and listing_count >= 3:
            return redirect(
                url_for(
                    "home",
                    listing_error="Basic accounts can list up to 3 products. Upgrade to Premium for unlimited listings."
                )
            )
        ...
        is_fast_food = (
            (vendor and vendor.get("seller_type") == "Fast Food")
            or session.get("role") == "Fast Food"
        )

        title = request.form.get(
            "meal_name" if is_fast_food else "title"
        )

        description = request.form.get(
            "meal_description" if is_fast_food else "description"
        )

        category = (
            "Fast Food"
            if is_fast_food
            else request.form.get("category", "Other")
        )
        ...
        # 👑 THE MASTER BUSINESS RULE FILTER
        if not is_fast_food and has_image == has_video:
            return redirect(
                url_for(
                    "home",
                    listing_error="Invalid Media Config: You must choose exactly one option—either an Item Cover Photo OR a maximum 20-second Showcase Video loop."
                )
            )
        ...
        if has_video:
            ...
            # 👑 ACTIVE TRIALS NOW PASS THE SAME PREMIUM VIDEO GATE
            # AS PAID PREMIUM STORES.
            if not premium_access:
                return redirect(
                    url_for(
                        "home",
                        listing_error="Only verified vendors with an active Premium Store or trial can upload product videos."
                    )
                )
            ...
            if video_extension not in VIDEO_EXTENSIONS:
                return redirect(
                    url_for(
                        "home",
                        listing_error="Product videos must be MP4, WebM, or MOV files."
                    )
                )
            ...
            try:
                ...
                if parsed_duration > 20.5:
                    ...
                    return redirect(
                        url_for(
                            "home",
                            listing_error="🚫 UPLOAD REFUSED: Showcase loops are strictly limited to a maximum of 20 seconds to keep load speeds instant for mobile users across Ghana."
                        )
                    )
        ...
        try:
            ...
            return redirect(
                url_for(
                    "home",
                    listing_error="Stock quantity must be a whole number greater than zero."
                )
            )
        ...
        if title and price and description and location:
            ...
            # 👑 ACTIVE TRIALS ALSO RECEIVE PREMIUM BUSINESS LABEL BENEFITS
            b_label = (
                session.get("company_name")
                if premium_access and session.get("company_name")
                else "Individual Vendor"
            )
    ...
    if company_search:
        product_conditions.append(
            "(business_label LIKE ? OR seller LIKE ?)"
        )
    ...
    logo_rows = query_db(
        "SELECT username, company_logo FROM users WHERE company_logo IS NOT NULL"
    ) or []
    ...
    if "cart" in session and session["cart"]:
        ...
        for item in items_in_db:
            ...
            seller_number = normalize_whatsapp_number(
    ...
    for seller_order in seller_orders.values():
        message = (
            f"Hello {seller_order['seller']}, "
            "I want to buy these products on Biz Hub:\n"
        )
        ...
        message += (
            f"\nTotal Cost: GH₵{seller_order['total']:.2f}. "
            "Let's arrange for payment and delivery."
        )

        seller_order["whatsapp_text"] = quote(message)

    premium_rows = query_db(
        "SELECT username FROM users WHERE (role = 'Vendor' OR role = 'Fast Food') AND plan = 'premium' AND subscription_expires_at > ?",
    ...
    } if premium_rows else set()

    trial_rows = query_db(
        "SELECT username FROM users WHERE (role = 'Vendor' OR role = 'Fast Food') AND plan = 'basic' AND subscription_expires_at > ?",
        (datetime.now(timezone.utc).isoformat(),)
    )
    ...
    } if trial_rows else set()

    premium_sellers.update(trial_sellers)

    for seller_order in seller_orders.values():
        seller_order["priority"] = (
            seller_order["seller"] in premium_sellers
        )
    ...
    if session.get("role") in ["Vendor", "Fast Food"]:
        ...
        listing_count = (
            listing_count_row["count"]
            if listing_count_row
            else 0
        )

        if session.get("role") == "Fast Food":
            fast_food_count_row = query_db(
                "SELECT COUNT(*) AS count FROM products WHERE seller = ? AND category = 'Fast Food'",
                (session["username"],),
                one=True
            )

            fast_food_count = (
                fast_food_count_row["count"]
                if fast_food_count_row
                else 0
            )
    ...
    return render_template(
        ...
        seller_orders=sorted(
            seller_orders.values(),
            key=lambda order: not order["priority"]
        ),
...
def add_to_cart(product_id):
    ...
    if product["stock_quantity"] < 1 or product["status"] == "Sold":
        return redirect(
            url_for(
                "home",
                listing_error="This product is sold out."
            )
        )
...
def mark_sold(product_id):
    ...
    product = query_db(
        "SELECT stock_quantity FROM products WHERE id = ? AND seller = ?",
    ...
    if product:
        ...
        if 1 <= sold_quantity <= product["stock_quantity"]:
            query_db(
                "UPDATE products SET stock_quantity = MAX(stock_quantity - ?, 0), status = CASE WHEN stock_quantity <= ? THEN 'Sold' ELSE 'Available' END WHERE id = ? AND seller = ?",
                (
                    sold_quantity,
                    sold_quantity,
                    product_id,
...
def place_order():
    ...
    items = query_db(
        f"SELECT * FROM products WHERE id IN ({placeholders}) AND stock_quantity > 0 AND status = 'Available'",
        cart_ids
    )
    ...
    if len(items) != len(cart_ids):
        return redirect(
            url_for(
                "home",
                listing_error="One or more cart items are no longer available."
            )
        )
...
def order_history():
    ...
    customer_orders = query_db(
        "SELECT * FROM orders WHERE customer_username = ? ORDER BY id DESC",
        (session["username"],)
    )
    ...
    seller_names = {
        row["username"]: (
            row["company_name"] or row["username"]
        )
        for row in query_db(
            "SELECT username, company_name FROM users"
        )
    }
...
def confirm_order(order_id):
    ...
    order_items = query_db(
        "SELECT * FROM order_items WHERE order_id = ? AND seller = ?",
        (
            order_id,
            session["username"]
        )
    ...
    if (
        order
        and order["payment_status"] == "Marked paid"
        and order["status"] == "Pending"
        and order_items
    ):
        for item in order_items:
            query_db(
                "UPDATE products SET stock_quantity = MAX(stock_quantity - ?, 0), status = CASE WHEN stock_quantity <= ? THEN 'Sold' ELSE 'Available' END WHERE id = ?",
                (
                    item["quantity"],
                    item["quantity"],
                    item["product_id"]
        ...
        query_db(
            "UPDATE orders SET status = 'Confirmed', payment_status = 'Confirmed' WHERE id = ?",
            (order_id,)
        )
...
def cancel_order(order_id):
    ...
    if query_db(
        "SELECT id FROM order_items WHERE order_id = ? AND seller = ?",
        (
            order_id,
            session["username"]
        )
...
def subscription():
    ...
    payment_number = normalize_whatsapp_number(
        os.environ.get(
            "BIZ_HUB_PAYMENT_WHATSAPP",
            "233558272972"
        )
    )

    payment_text = quote(
        f"Hello Biz Hub, I want to upgrade my {session['username']} account to Premium Store."
    )
...
def request_premium():
    if (
        ...
        or session.get("role") not in ["Vendor", "Fast Food"]
    ):
        return redirect(url_for("login"))
...
def admin_login():
    if request.method == "POST":
        if not admin_configured():
            return render_template(
                "admin_login.html",
                admin_error="Admin credentials are not configured."
            )

        submitted_username = request.form.get(
            "username",
            ""
        ).strip()
        ...
        configured_login = (
            submitted_username
            == os.environ.get(
                "BIZ_HUB_ADMIN_USERNAME",
                ""
            ).strip()
            and submitted_password
            == os.environ.get(
                "BIZ_HUB_ADMIN_PASSWORD",
                ""
            )
        )

        local_login = (
            submitted_username == LOCAL_ADMIN_USERNAME
            and submitted_password == LOCAL_ADMIN_PASSWORD
        )
    ...
    return render_template(
        ...
        signup_available=admin_signup_available()
    )
...
def admin_signup():
    ...
    if request.method == "POST":
        ...
        if not username or not password:
            return render_template(
                "admin_signup.html",
                admin_error="Username and password are required."
            )

        if password != confirm_password:
            return render_template(
                "admin_signup.html",
                admin_error="The passwords do not match."
            )
        ...
        try:
            ...
            return render_template(
                "admin_signup.html",
                admin_error="That admin username is already taken."
            )
...
def admin_dashboard():
    ...
    users = query_db(
        "SELECT * FROM users ORDER BY COALESCE(registered_at, '') DESC, username"
    )

    listing_counts = {
        row["seller"]: row["count"]
        for row in query_db(
            "SELECT seller, COUNT(*) AS count FROM products GROUP BY seller"
        )
    }

    ledger_entries = query_db(
        "SELECT * FROM financial_ledger ORDER BY id DESC"
    ) or []

    total_rev_row = query_db(
        "SELECT SUM(amount) AS total FROM financial_ledger WHERE status = 'Verified'",
        one=True
    )
    ...
    total_revenue = (
        total_rev_row["total"]
        if total_rev_row
        and total_rev_row["total"] is not None
        else 0.0
    )

    pending_momo_row = query_db(
        "SELECT SUM(amount) AS total FROM financial_ledger WHERE status = 'Pending'",
        one=True
    )

    pending_momo = (
        pending_momo_row["total"]
        if pending_momo_row
        and pending_momo_row["total"] is not None
        else 0.0
    )

    verified_count_row = query_db(
        "SELECT COUNT(*) AS count FROM financial_ledger WHERE status = 'Verified'",
        one=True
    )

    verified_count = (
        verified_count_row["count"]
        if verified_count_row
        and verified_count_row["count"] is not None
        else 0
    )
...
def verify_transaction(entry_id):
    ...
    query_db(
        "UPDATE financial_ledger SET status = 'Verified' WHERE id = ?",
        (entry_id,)
    )
...
def log_payment():
    ...
    ref = (
        ...
        or f"WA-{uuid.uuid4().hex[:8].upper()}"
    )
...
def approve_premium(user_id):
    ...
    query_db(
        "UPDATE users SET plan = 'premium', subscription_expires_at = ?, upgrade_requested_at = NULL WHERE id = ? AND (role = 'Vendor' OR role = 'Fast Food')",
        (
            expiry.isoformat(),
            user_id
        )
...
def admin_delete_user(user_id):
    ...
    product_rows = query_db(
        "SELECT image_file, video_file FROM products WHERE seller = ?",
        (user["username"],)
    ) or []
    ...
    order_ids = {
        row["order_id"]
        for row in query_db(
            "SELECT order_id FROM order_items WHERE seller = ?",
            (user["username"],)
        )
        if row
    }
    ...
    order_ids.update(
        row["id"]
        for row in query_db(
            "SELECT id FROM orders WHERE customer_username = ?",
            (user["username"],)
        )
        if row
    )
    ...
    query_db(
        "DELETE FROM order_items WHERE seller = ?",
        (user["username"],)
    )
    ...
    query_db(
        "DELETE FROM products WHERE seller = ?",
        (user["username"],)
    )

    query_db(
        "DELETE FROM vendor_categories WHERE user_id = ?",
        (user["id"],)
    )
...
def settings():
    ...
    if request.method == "POST":
        ...
        logo_upload = request.files.get(
            "company_logo"
        )

        catalog_mode = request.form.get(
            "catalog_mode",
            "Focused"
        )
        ...
        selected_categories = [
            category
            for category in request.form.getlist(
                "vendor_categories"
            )
        ...
        if not email:
            return render_template(
                "settings.html",
                user=user,
                settings_error="Email is required."
            )
        ...
        if is_vendor_any and not whatsapp_number:
            return render_template(
                "settings.html",
                user=user,
                settings_error="Vendor accounts need a WhatsApp number for payments."
            )
        ...
        if new_password and new_password != confirm_password:
            return render_template(
                "settings.html",
                user=user,
                settings_error="The new passwords do not match."
            )

        if logo_upload and logo_upload.filename:
            company_logo = save_company_logo(
                logo_upload
            )
            ...
            if not company_logo:
                return render_template(
                    ...
                    settings_error="Upload a PNG, JPG, JPEG, WEBP, or GIF logo."
                )
        ...
        if not is_vendor_any:
            ...
            catalog_mode not in ("Variety", "Focused")
            or not selected_categories
        ):
            return render_template(
                "settings.html",
                user=user,
                vendor_categories=vendor_categories,
                vendor_category_options=VENDOR_CATEGORIES,
                settings_error="Choose whether you sell a variety or focus on a category, then select at least one product range."
            )
        ...
        for category in selected_categories:
            query_db(
                "INSERT INTO vendor_categories (user_id, category) VALUES (?, ?)",
                (
                    user["id"],
                    category
                )
...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        if not user:
            reset_error = (
                "No account was found with that registered WhatsApp number."
            )
            ...
            reset_text = quote(
                f"Hello Biz Hub, I need to recover my account registered with WhatsApp {whatsapp_number}. My reset link is: {reset_link}"
            )

            reset_link = (
                f"https://wa.me{payment_number}?text={reset_text}"
            )
...
def reset_credentials(token):
    ...
    if not reset:
        return render_template(
            ...
            reset_error="This recovery link is invalid or has expired.",
            token=None
        )
    ...
    if request.method == "POST":
        ...
        if not new_username or not new_password:
            return render_template(
                "reset_credentials.html",
                reset_error="Username and password are required.",
                token=token,
                user=user
            )
        ...
        if new_password != confirm_password:
            return render_template(
                "reset_credentials.html",
                reset_error="The passwords do not match.",
                token=token,
                user=user
            )
        ...
        try:
            ...
            return render_template(
                "reset_credentials.html",
                reset_error="That username is already taken.",
                token=token,
                user=user
            )
...
def register():
    ...
    if request.method == "POST":
        ...
        submitted_role = request.form.get(
            "role",
            ""
        ).strip()
        ...
        if not username or not email or not password:
            return render_template(
                "login.html",
                reg_error="Username, email, and password are required."
            )
        ...
        if logo_upload and logo_upload.filename:
            company_logo_filename = save_company_logo(
                logo_upload
            )
        ...
        if role in ["Vendor", "Fast Food"] and not whatsapp_number:
            return render_template(
                "login.html",
                reg_error="Merchant and Fast Food vendor accounts need a compulsory WhatsApp number to receive order tallies."
            )
        ...
        if role == "Vendor" and (
            ...
            return render_template(
                "login.html",
                reg_error="Choose a product range and select at least one category."
            )
...
# 👑 MASTER VIDEO CHUNK STREAMING SYSTEM
...
def stream_video(filename):
    ...
    byte_range = request.headers.get(
        "Range",
        None
    )
    ...
    parsed_range = re.search(
        r"bytes=(\d+)-(\d*)",
        byte_range
    )

    start_byte = int(
        parsed_range.group(1)
    )

    end_byte = (
        int(parsed_range.group(2))
        if parsed_range.group(2)
        else file_size - 1
    )
