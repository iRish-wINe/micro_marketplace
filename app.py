        ...
        return render_template_string("""
            ...
            <body>
                <div class="card">
                    ...
                    <p>
                        To preserve data session encryption, secure vendor verification
                        pipelines, and modern layout alignment, Biz Hub no longer supports
                        devices running Android 7.0 or below.
                    </p>
                    <p>
                        Please upgrade your browser application or system software to
                        restore full commercial privileges.
                    </p>
                    <div class="footer-note">
                        Biz Hub Operations • Secure Marketplace Infrastructure
                    </div>
                </div>
            </body>
            </html>
...
def normalize_whatsapp_number(number):
    digits = "".join(
        character for character in (number or "")
...
def subscription_status(user):
    ...
    # Active paid Premium OR active promotional trial.
    premium_active = (
        is_vendor_role
        and (
            (
...
def admin_configured():
    return bool(
        get_admin_username() and get_admin_password()
    ) or bool(
...
def home():
    if request.method == "POST":
        ...
        if (
            not vendor_subscription["is_premium"]
            and listing_count >= 3
        ):
            return redirect(
                url_for(
                    "home",
                    listing_error=(
                        "Basic accounts can list up to 3 products. "
        ...
        is_fast_food = (
            vendor
            and vendor.get("seller_type") == "Fast Food"
        ...
        if not is_fast_food and has_image == has_video:
            return redirect(
                url_for(
                    "home",
                    listing_error=(
                        "Invalid Media Config: You must choose exactly "
                        "one option—either an Item Cover Photo OR a "
                        "maximum 20-second Showcase Video loop."
                    )
                )
            )
        ...
        # ==========================================================
        # PRODUCT IMAGE FIX
        # Generate a unique filename instead of using the original
        # uploaded filename. This prevents filename collisions and
        # broken/overwritten published images.
        # ==========================================================
        if has_image:
            ...
            if image_extension not in {
                ...
                return redirect(
                    url_for(
                        "home",
                        listing_error=(
                            "Product images must be PNG, JPG, JPEG, "
                            "WEBP, or GIF files."
                        )
                    )
                )
            ...
            filename = (
                f"image-{uuid.uuid4().hex}"
                f"{image_extension}"
            )
        else:
            filename = (
                "fast-food-placeholder.svg"
                if is_fast_food
                else ""
            )
        ...
        if has_video:
            ...
            # Active Premium OR active trial has is_premium=True.
            if not vendor_subscription["is_premium"]:
                return redirect(
                    url_for(
                        "home",
                        listing_error=(
                            "Only verified vendors with an active "
                            "Premium Store or trial can upload "
                            "product videos."
                        )
                    )
                )
            ...
            if video_extension not in VIDEO_EXTENSIONS:
                return redirect(
                    url_for(
                        "home",
                        listing_error=(
                            "Product videos must be MP4, WebM, "
                            "or MOV files."
                        )
                    )
                )
                ...
                if parsed_duration > 20.5:
                    ...
                    return redirect(
                        url_for(
                            "home",
                            listing_error=(
                                "🚫 UPLOAD REFUSED: Showcase loops "
                                "are strictly limited to a maximum "
                                "of 20 seconds to keep load speeds "
                                "instant for mobile users across Ghana."
                            )
                        )
                    )
        ...
        if title and price and description and location:
            ...
            # ======================================================
            # PRODUCT IMAGE SAVE
            # Uses the UUID filename generated above.
            # ======================================================
            ...
            b_label = (
                ...
                if (
                    vendor_subscription["is_premium"]
                    and session.get("company_name")
                )
    ...
    listing_error = request.args.get(
        "listing_error"
    )
    ...
    logo_rows = query_db(
        "SELECT username, company_logo FROM users "
    ...
    if "cart" in session and session["cart"]:
        ...
        for item in items_in_db:
            ...
            seller_order["items"].append(
                item
            )

            seller_order["total"] += float(
                item["price"]
            )
    ...
    for seller_order in seller_orders.values():
        ...
        message += (
            f"\nTotal Cost: "
            f"GH₵{seller_order['total']:.2f}. "
            "Let's arrange for payment and delivery."
        )

        seller_order["whatsapp_text"] = quote(
            message
        )
    ...
    if session.get("role") in [
        ...
        vendor_subscription = subscription_status(
            vendor
        )
...
def delete_item(product_id):
    ...
    if (
        product
        and product["seller"] == session["username"]
    ):
        query_db(
...
@app.route(
    "/orders/<int:order_id>/payment-sent",
    methods=["POST"]
)
def mark_payment_sent(order_id):
    ...
    query_db(
        """
        UPDATE orders
        SET payment_status = 'Marked paid'
        WHERE id = ?
...
def subscription():
    ...
    payment_text = quote(
        f"Hello Biz Hub, I want to upgrade my "
        f"{session['username']} account to Premium Store."
    )
...
@app.route(
    "/admin/verify-transaction/<int:entry_id>",
    methods=["POST"]
)
...
@app.route(
    "/admin/approve-premium/<int:user_id>",
    methods=["POST"]
)
    ...
    if request.method == "POST":
        ...
        if is_vendor_any and not whatsapp_number:
            return render_template(
                ...
                user=user,
                settings_error=(
                    "Vendor accounts need a WhatsApp number "
                    "for payments."
                )
            )
        ...
        if not is_vendor_any:
            ...
            return render_template(
                ...
                vendor_category_options=VENDOR_CATEGORIES,
                settings_error=(
                    "Choose whether you sell a variety or "
                    "focus on a category, then select at "
                    "least one product range."
                )
            )
...
def forgot_password():
    ...
    if request.method == "POST":
        ...
        if not user:
            ...
            reset_text = quote(
                f"Hello Biz Hub, I need to recover "
                f"my account registered with WhatsApp "
                f"{whatsapp_number}. My reset link is: "
                f"{reset_link}"
            )

            reset_link = (
                f"https://wa.me{payment_number}"
                f"?text={reset_text}"
            )
...
def register():
    ...
    if request.method == "POST":
        ...
        if (
            role in ["Vendor", "Fast Food"]
            and not whatsapp_number
        ):
            return render_template(
                "login.html",
                reg_error=(
                    "Merchant and Fast Food vendor accounts "
                    "need a compulsory WhatsApp number to "
                    "receive order tallies."
                )
            )
        ...
        if (
            ...
            return render_template(
                "login.html",
                reg_error=(
                    "Choose a product range and select "
        ...
        try:
            ...
            # ======================================================
            # TWO-MONTH NEW-REGISTRATION TRIAL
            # ======================================================
            trial_expires_at = (
                trial_started_at
                + timedelta(days=60)
...
# Fixes blank video screens on mobile browsers
# ==============================================================
